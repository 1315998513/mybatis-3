...
public class Configuration {
  ...
  public Executor newExecutor(Transaction transaction, ExecutorType executorType) {
    ...
    /**
     * SimpleExecutor extends (BaseExecutor implements Executor) SimpleExecutor是对jdbc的封装
     * 底层执行器中查询中，获取数据会先去查缓存，返回为null时执行查询数据库
     * 底层执行器方法中修改的方法会清空一级缓存
     */
    else {
      executor = new SimpleExecutor(this, transaction);
    }
    // mybatis默认开启的一级缓存开关，创建缓存执行器
    if (cacheEnabled) {
